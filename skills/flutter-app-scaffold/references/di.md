...
Кладётся в `lib/src/common/core/di/`. Контейнер — деталь за абстракцией: тут обёртка над `kiwi`, сменяема, код этого не заметит.
...
Берёт `AppConfig`, регистрирует реализации в `Registrar` и возвращает `Resolver`.
