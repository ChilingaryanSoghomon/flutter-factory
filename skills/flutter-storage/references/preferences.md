...
При сборке регистрируем в DI-контейнере `KeyValueStorage` (реализация `KeyValueStorageImpl`) и фасад `SharedPrefsStorage`. Реализации нужен готовый `SharedPreferences` — асинхронный, получи до регистрации.
