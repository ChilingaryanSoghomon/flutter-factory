...
При сборке регистрируем в DI-контейнере `AppDatabase` (открывается асинхронно) и `NotesDao` (реализация `NotesDaoImpl` поверх соединения `AppDatabase`). Схема и миграции (`version` + `onUpgrade`) живут в этой папке, запрос — внутри DAO.
