...
Один раз при сборке приложения. `AppDatabase` контракта не имеет — открывается (асинхронно) и кладётся в контейнер под своим типом. `NotesDao` регистрируют под типом контракта, а объектом дают реализацию `NotesDaoImpl` поверх соединения `AppDatabase`. Как класть в контейнер — это часть про контейнер; дальше потребители берут `NotesDao` оттуда. Схема и миграции (`version` + `onUpgrade`) — деталь этой папки; запрос живёт внутри DAO, наружу торчит только контракт `NotesDao`.
